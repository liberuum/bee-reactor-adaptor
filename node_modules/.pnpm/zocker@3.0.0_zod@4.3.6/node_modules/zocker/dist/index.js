...
const uuid_generator = (schema, ctx) => {
	...
	if (version == 4 || !version) return faker.string.uuid();
	const pattern = schema._zod.def.pattern ?? z$2.regexes.uuid(version);
...
const guid_generator = (schema, ctx) => {
	const pattern = schema._zod.def.pattern ?? z$2.regexes.guid;
	const randexp = new Randexp(pattern);
	randexp.randInt = (min, max) => faker.number.int({
		min,
		max
...
const e164_generator = (schema, ctx) => {
	const pattern = getRegexCheck$1(schema) ?? schema._zod.def.pattern ?? z$2.regexes.e164;
	const randexp = new Randexp(pattern);
	randexp.randInt = (min, max) => faker.number.int({
		min,
		max
...
function getRegexCheck$1(schema) {
	const checks = schema._zod.def.checks ?? [];
	const regex_checks = checks.filter((check) => check instanceof z$2.$ZodCheckRegex);
...
const email_generator = (schema, ctx) => {
	const pattern = getRegexCheck(schema) ?? schema._zod.def.pattern ?? z$2.regexes.email;
	const generated_email = faker.internet.email();
...
const iso_datetime_generator = (schema, ctx) => {
	...
	let precision = defined_precision != null ? defined_precision : faker.number.int({
		min: 0,
		max: 6
	});
	let datetime$1 = faker.date.recent({ days: 100 }).toISOString();
	...
	if (offset) {
		const hours_number = faker.number.int({
			min: 0,
			max: 23
		});
		const minutes_number = faker.number.int({
			min: 0,
			max: 59
		});
		const hours = hours_number.toString().padStart(2, "0");
		const minutes = minutes_number.toString().padStart(2, "0");
		const sign = faker.datatype.boolean({ probability: .5 }) ? "+" : "-";
...
const nanoid_generator = (schema, ctx) => {
	const pattern = z$2.regexes.nanoid;
	const randexp = new Randexp(pattern);
	randexp.randInt = (min, max) => faker.number.int({
		min,
		max
...
const ulid_generator = (schema, ctx) => {
	...
	for (let i = 0; i < generated_length; i++) ulid$1 += faker.helpers.arrayElement(ULID_CHARS);
...
const cidrv4_generator = (schema, ctx) => {
	...
	else mask_length = faker.number.int({
		min: 1,
		max: 2
	});
	const mask = mask_length == 1 ? faker.number.int({
		min: 0,
		max: 9
	}) : faker.number.int({
		min: 10,
...
const cidrv6_generator = (schema, ctx) => {
	...
	const mask = mask_length == 1 ? faker.number.int({
		min: 0,
		max: 9
	}) : mask_length == 2 ? faker.number.int({
		min: 10,
		max: 99
	}) : faker.number.int({
		min: 100,
...
function generateURLSafeStringForRegex(regex$1) {
	...
	randexp.defaultRange.add(65, 90);
	randexp.randInt = (min, max) => faker.number.int({
		min,
		max
	});
...
const pipe_generator = (schema, ctx) => {
	...
	if (!(second instanceof z$2.$ZodTransform)) return generate$1(second, ctx);
...
const generate_emoji = (schema, ctx) => {
	...
	for (let i = 0; i < length; i++) emojis += faker.internet.emoji();
...
function generateBase64String() {
	...
	let data = "";
	for (let i = 0; i < bytes; i++) data += String.fromCharCode(faker.number.int({
		min: 0,
		max: 255
	}));
	return btoa(data);
...
const ksuid_generator = (schema, ctx) => {
	...
	const content_constraints$1 = getContentConstraints(schema);
	const ksuid = new Randexp(z$2.regexes.ksuid);
	return ksuid.gen();
};
...
const xid_generator = (schema, ctx) => {
	const pattern = schema._zod.def.pattern ?? z$2.regexes.xid;
	const length_constraints$1 = getLengthConstraints(schema);
...
const OVERRIDE_NAMES$1 = {
	number: z$2.$ZodNumber,
	string: z$2.$ZodString,
	boolean: z$2.$ZodBoolean,
	bigint: z$2.$ZodBigInt,
	date: z$2.$ZodDate,
	undefined: z$2.$ZodUndefined,
	null: z$2.$ZodNull,
	any: z$2.$ZodAny,
	unknown: z$2.$ZodUnknown,
	void: z$2.$ZodVoid,
	never: z$2.$ZodNever,
	array: z$2.$ZodArray,
	object: z$2.$ZodObject,
	union: z$2.$ZodUnion,
	intersection: z$2.$ZodIntersection,
	tuple: z$2.$ZodTuple,
	record: z$2.$ZodRecord,
	map: z$2.$ZodMap,
	set: z$2.$ZodSet,
	lazy: z$2.$ZodLazy,
	literal: z$2.$ZodLiteral,
	enum: z$2.$ZodEnum,
	promise: z$2.$ZodPromise,
	transformer: z$2.$ZodTransform,
	optional: z$2.$ZodOptional,
	nullable: z$2.$ZodNullable
};
...
function weighted_pick(option_1, option_2, probability) {
	...
	const random = faker.number.float({
		min: 0,
		max: 1
	});
...
const jsonSchema = z$1.lazy(() => z$1.union([
	literalSchema,
	z$1.array(jsonSchema),
	z$1.record(jsonSchema)
]));
const any$1 = z$1.any();
...
const generate_object = (object_schema, ctx) => {
	...
	if (catchall_schema instanceof z$1.ZodNever) catchall_schema = null;
	const is_passthrough = object_schema._def.unknownKeys === "passthrough";
	if (is_passthrough && !catchall_schema) catchall_schema = z$1.any();
	if (catchall_schema && ctx.object_options.generate_extra_keys) {
		const key_schema = z$1.union([
			z$1.string(),
			z$1.number(),
			z$1.symbol()
		]);
		const num_additional_keys = faker.number.int({
			min: 0,
			max: 10
		});
		try {
...
const DiscriminatedUnionGenerator = {
	schema: z$1.ZodDiscriminatedUnion,
...
const generate_array = (array_schema, ctx) => {
	...
	const length = exact_length !== null ? exact_length : faker.number.int({
		min,
		max
	});
	const generated_array = [];
...
const ip = (ctx, lc, cc, td) => {
	...
	return weighted_random_boolean(.5) ? faker.internet.ipv4() : faker.internet.ipv6();
};
const datetime = (ctx, lc, cc, td) => {
	const format = cc.format;
	const offset = format.offset === true;
	let datetime$1 = faker.date.recent().toISOString();
	if (offset) {
		const hours_number = faker.number.int({
			min: 0,
			max: 23
...
const emoji = (ctx, lc, cc, td) => {
	const length = lc.exact ?? faker.number.int({
		min: lc.min ?? 0,
...
const any = (ctx, lc, cc, td) => {
	...
	let length = lc.exact ?? faker.number.int({
		min,
		max
	});
	...
	return (cc.starts_with ?? "") + faker.string.sample(generated_length) + cc.includes.join() + (cc.ends_with ?? "");
};
function color() {
	const generators = [faker.color.human, faker.color.rgb];
...
const merge_schema = (schema_1, schema_2) => {
	if (schema_1 instanceof z$1.ZodNumber && schema_2 instanceof z$1.ZodNumber) {
		const combined = z$1.number();
		combined._def.checks = [...schema_1._def.checks, ...schema_2._def.checks];
		return combined;
	}
	if (schema_1 instanceof z$1.ZodString && schema_2 instanceof z$1.ZodString) {
		const combined = z$1.string();
		combined._def.checks = [...schema_1._def.checks, ...schema_2._def.checks];
		return combined;
	}
	if (schema_1 instanceof z$1.ZodBoolean && schema_2 instanceof z$1.ZodBoolean) return z$1.boolean();
	if (schema_1 instanceof z$1.ZodLiteral && schema_2 instanceof z$1.ZodLiteral) if (schema_1._def.value === schema_2._def.value) return schema_1;
	else throw new InvalidSchemaException("There is no intersection between two literals with different values.");
	if (schema_1 instanceof z$1.ZodSymbol && schema_2 instanceof z$1.ZodSymbol) return z$1.symbol();
	throw new NoGeneratorException("ZodIntersections only have very limited support at the moment.");
};
const IntersectionGenerator = {
	schema: z$1.ZodIntersection,
...
const OVERRIDE_NAMES = {
	...
	string: z$1.ZodString,
	boolean: z$1.ZodBoolean,
	bigint: z$1.ZodBigInt,
	date: z$1.ZodDate,
	undefined: z$1.ZodUndefined,
	null: z$1.ZodNull,
	any: z$1.ZodAny,
	...
	never: z$1.ZodNever,
	array: z$1.ZodArray,
	object: z$1.ZodObject,
	union: z$1.ZodUnion,
	intersection: z$1.ZodIntersection,
	tuple: z$1.ZodTuple,
	record: z$1.ZodRecord,
	map: z$1.ZodMap,
	set: z$1.ZodSet,
	lazy: z$1.ZodLazy,
	literal: z$1.ZodLiteral,
	enum: z$1.ZodEnum,
	nativeEnum: z$1.ZodNativeEnum,
	promise: z$1.ZodPromise,
	transformer: z$1.ZodTransformer,
	optional: z$1.ZodOptional,
	nullable: z$1.ZodNullable,
	effects: z$1.ZodEffects
};
