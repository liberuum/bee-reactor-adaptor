...
/** Builder-tools can inject the literal string "null" when no URL is configured; skip fetch in that case. */
function isUsableRegistryUrl(url: string | null): url is string {
...
export function useRegistryPackages() {
  ...
  useEffect(() => {
    async function refreshPackages() {
      if (!isUsableRegistryUrl(PH_PACKAGE_REGISTRY_URL) || !packageManager) return;
