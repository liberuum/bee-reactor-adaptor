 ...
 */
export function buildSignMessage(address, origin) {
    return [
 ...
 */
export async function loadCachedSwarmKey(address) {
    try {
 ...
 */
export async function cacheSwarmKey(entry) {
    const db = await openSwarmKeyDB();
    return new Promise((resolve, reject) => {
 ...
 */
export async function clearCachedSwarmKey() {
    try {
        const db = await openSwarmKeyDB();
        return new Promise((resolve, reject) => {
            const tx = db.transaction(SWARM_KEY_STORE_NAME, "readwrite");
 ...
 */
export async function getOrDeriveSwarmKey(address, origin) {
    // Check cache first
    try {
