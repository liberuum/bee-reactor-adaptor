...
import { ensureChatPeerInUserManifest, clearChatPeersInUserManifest, } from "../channel/manifest-manager.js";
...
async function initChatManager(beeUrl, plugin, stampBatchId) {
    ...
    try {
        ...
        const collabManager = new CollabManager(swarmClient, chatManager, ownerAddress, collabGsoc);
...
export async function initSwarmPlugin() {
    ...
    if (!usableStamp) {
        ...
        if (ph.swarm) {
            ph.swarm.client = {
                createStamp: async (amount, depth, options) => {
                    const headers = {
                ...
                getStampOptions: async () => {
                    ...
                    if (!res.ok)
                        throw new Error("Failed to get chainstate");
...
function applySwarmExtensions(ph, isDevMode) {
    ...
    ph.swarm.clearChats = async () => {
        ...
        try {
            ...
            console.warn("[SwarmPlugin] clearChats: manifest clear failed:", err instanceof Error ? err.message : err);
        }
