...
export class SwarmChannel {
    ...
    async withManifestLock(docId, fn) {
    ...
    async handleOutboxAdded(syncOps) {
        ...
        for (const syncOp of syncOps) {
            try {
                ...
                if ((maxOrdinal > 0 && maxOrdinal <= this.initOutboxCursor) ||
                    ...
                    this.logger.info(`[SwarmChannel] Skipped ${this.recoveryInProgress ? "recovery" : "already-synced"} ops for ${syncOp.documentId.slice(0, 8)} (ordinal ${maxOrdinal}, cursor ${this.initOutboxCursor})`);
                    continue;
                }
                syncOp.started();
    ...
    async pushSyncOperation(syncOp) {
        ...
        const isDriveDelete = docType === "powerhouse/document-drive" &&
        ...
        if (driveId && !isDriveDelete) {
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                try {
                    ...
                    if (attempt < 2) {
                        this.logger.warn(`[SwarmChannel] Manifest update retry ${attempt + 1}/3 for drive ${driveId.slice(0, 8)}: ${err instanceof Error ? err.message : err}`);
                        await new Promise((r) => setTimeout(r, 1000 * (attempt + 1)));
                    }
                    else {
                        this.logger.error(`[SwarmChannel] Manifest update FAILED for drive ${driveId.slice(0, 8)} after 3 attempts — recovery may miss documents: ${err instanceof Error ? err.message : err}`);
                    }
                }
            }
        }
    ...
    async pullFromSwarm() {
        ...
        if (docIds.size === 0) {
            this.logger.warn(`[SwarmChannel] pullFromSwarm: all docs filtered out as local-present (nothing to recover)`);
            return false;
        }
        this.logger.info(`[SwarmChannel] Recovery: ${docIds.size} docs to pull: ${[...docIds].map(id => id.slice(0, 8)).join(", ")}`);
        ...
        const skippedDeletedDrives = new Set();
        let newOpsCount = 0;
        for (const docId of orderedDocIds) {
            ...
            try {
                ...
                for (const [, ops] of byScope) {
                    ops.sort((a, b) => (a.operation?.index ?? 0) - (b.operation?.index ?? 0));
                }
                ...
                if (docTypeForId === "powerhouse/document-drive") {
                    ...
                    if (hasDelete) {
                        ...
                        try {
                            ...
                            this.logger.warn(`[SwarmChannel] Cleanup failed for deleted drive ${docId.slice(0, 8)}: ${err instanceof Error ? err.message : err}`);
                        }
                        continue;
                    }
                }
                for (const scope of scopeOrder) {
                    ...
                    if (!scopeOps || scopeOps.length === 0)
                        continue;
                    const syncOp = new SyncOperation(crypto.randomUUID(), "", [], this.remoteName, docId, [scope], branch, scopeOps);
        ...
        if (newOpsCount > 0) {
            ...
            setTimeout(() => {
                ...
                const maxSeen = Math.max(this.outbox.ackOrdinal, this.outbox.latestOrdinal, this.recoveryMaxOrdinal);
