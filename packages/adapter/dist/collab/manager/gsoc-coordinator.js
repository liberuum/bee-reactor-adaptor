...
export class GsocCoordinator {
    ...
    gsoc, store, events, applyPipeline, pollKick) {
        this.client = client;
     ...
     * cached mined signer.
     */
    async provisionOutbound(collabId, peers) {
        ...
        for (const peer of peers) {
            if (this.isSelf(peer.address))
                continue;
            const key = keyFor(collabId, peer.address);
            if (this.outbound.has(key) && collabMap[peer.address])
                continue;
     ...
     */
    async subscribeToPeers(summary) {
        if (!this.gsoc)
            return;
        for (const peer of summary.participants) {
            ...
            try {
                ...
                const subscription = this.gsoc.subscribeWithIdentifier(subKey, listenAddress, identifierHex, {
                    ...
                    onNotification: (n) => this.handlePing(this.store.get(summary.collabId) ?? summary, peer.address, n),
                    onError: (err) => {
     ...
     */
    async pingOpCommitted(summary, driveId, docId, write) {
        if (!this.gsoc)
            return;
        ...
        for (const peer of summary.participants) {
            ...
            if (!mined)
                continue; // haven't mined yet — next push will catch up
            try {
     ...
     */
    handlePing(summary, writerAddress, notification) {
    ...
    async lookupOverlay(peerAddress) {
        try {
...
function keyFor(collabId, peerAddress) {
    return `${collabId}:${peerAddress}`;
}
//# sourceMappingURL=gsoc-coordinator.js.map