 ...
 */
import { ensureCollabInUserManifest, listCollabsFromUserManifest, } from "../../channel/manifest-manager.js";
export class UserManifestSync {
    ...
    manifestFeed;
    constructor(client, myAddress, store, events, gsoc) {
        this.client = client;
    ...
    async upsert(summary) {
        await ensureCollabInUserManifest(this.client, this.myAddress, this.toManifestEntry(summary));
    }
    /**
    ...
    async rehydrate() {
        ...
        try {
            ...
            for (const entry of entries) {
                if (this.store.has(entry.collabId))
                    continue;
     ...
     * will promote it.
     */
    async reconstructSummary(entry) {
        if (!this.manifestFeed) {
