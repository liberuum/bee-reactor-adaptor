...
import { getReactorClient, listDocIdsInDrive, } from "./reactor-bridge.js";
...
export class PollLoop {
    ...
    notFoundAt = new Map();
    constructor(store, opsFeed, applyPipeline, gsoc, myAddress, isShuttingDown) {
        this.store = store;
    ...
    async pollPeerDoc(summary, docId, participant) {
        try {
            ...
            const cursor = this.store.readCursor(summary.collabId, participant.address, docId);
            if (cursor > 0) {
                ...
                if (lastMiss && Date.now() - lastMiss < FEED_404_TTL_MS)
                    return;
            }
...
function notFoundKey(collabId, peerAddress, docId) {
