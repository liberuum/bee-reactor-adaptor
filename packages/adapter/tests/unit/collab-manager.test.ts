 ...
 *
 * Network-dependent flows (create, accept, revoke, provisionOutboundGsoc,
 * pollSummary's real feed reads) are covered by integration tests; this
 * file runs with zero Bee dependency so it can ship in CI without a node.
 */
...
describe("CollabManager — state + persistence", () => {
  ...
  it("persists summaries to localStorage and loads them back on reconstruction", () => {
    ...
    (mgr1 as any).summaries.set(s.collabId, s);
    (mgr1 as any).persist();
    mgr1.shutdown();
  ...
  it("list() returns summaries sorted by lastActivityAt descending", () => {
    ...
    (mgr as any).summaries.set(old.collabId, old);
    (mgr as any).summaries.set(recent.collabId, recent);
    (mgr as any).summaries.set(mid.collabId, mid);
...
describe("CollabManager — events", () => {
  ...
  it("on('*') receives all event types", () => {
    ...
    (mgr as any).emit({ type: "collab-created", collabId: "a" });
    (mgr as any).emit({ type: "collab-updated", collabId: "a" });
    (mgr as any).emit({ type: "collab-removed", collabId: "a" });
...
// ─── applyOpsAndAdvanceCursor ─────────────────────────────────────

describe("CollabManager — applyOpsAndAdvanceCursor", () => {
  ...
  it("returns false when reactor.load is unavailable", async () => {
    ...
    const mgr = new CollabManager(makeStubClient(), makeStubChat(), TEST_ADDRESS);
    const ok = await (mgr as any).applyOpsAndAdvanceCursor({
      collabId: "drive:x",
      writer: PEER_ADDRESS,
      docId: "doc-1",
      branch: "main",
...
// ─── handlePeerPing routing ──────────────────────────────────────

describe("CollabManager — handlePeerPing tier routing", () => {
  let loadCalls: Array<{ docId: string; branch: string; ops: unknown[] }>;
  ...
  it("inline tier: inlineOps in ping → applied immediately (no Swarm fetch)", async () => {
    ...
    (mgr as any).handlePeerPing(s, PEER_ADDRESS, notification);
  ...
  it("refs tier: refs only → applyFromPing triggered (no inline)", async () => {
    ...
    (mgr as any).summaries.set(s.collabId, s);
    const spy = vi
      .spyOn(mgr as any, "applyFromPing")
  ...
  it("feed tier: no refs, no inline → triggers pollOnce", async () => {
    const mgr = new CollabManager(makeStubClient(), makeStubChat(), TEST_ADDRESS);
    const s = mkSummary();
    (mgr as any).summaries.set(s.collabId, s);
    const spy = vi.spyOn(mgr as any, "pollOnce").mockResolvedValue(undefined);
  ...
  it("ignores pings for a different collab", async () => {
    ...
    (mgr as any).summaries.set(s.collabId, s);
    const applyInline = vi.spyOn(mgr as any, "applyInlineOps").mockResolvedValue(undefined);
...
describe("CollabManager — handleLocalPush routing", () => {
  ...
  it("no-op when no summary matches the driveId", async () => {
    const mgr = new CollabManager(makeStubClient(), makeStubChat(), TEST_ADDRESS);
    const spy = vi.spyOn((mgr as any).opsFeed, "appendBatch");
    await mgr.handleLocalPush({
      driveId: "unrelated-drive",
      docId: "doc-x",
      ops: [{ operation: { id: "op1", index: 0 } }],
      scope: "global",
      branch: "main",
    });
  ...
  it("drive-level collab catches any doc in the drive", async () => {
    ...
    (mgr as any).summaries.set(s.collabId, s);
    const spy = vi
      .spyOn((mgr as any).opsFeed, "appendBatch")
  ...
  it("mirror-push failure is logged but doesn't throw to the caller", async () => {
    ...
    (mgr as any).summaries.set(s.collabId, s);
    vi.spyOn((mgr as any).opsFeed, "appendBatch").mockRejectedValue(new Error("nope"));
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
...
describe("CollabManager — leave + shutdown", () => {
  ...
  it("shutdown() clears the global hook + polling timer", () => {
    const mgr = new CollabManager(makeStubClient(), makeStubChat(), TEST_ADDRESS);
    expect((globalThis as any).__swarmCollabManager__).toBe(mgr);
  ...
  it("shutdown() cancels inbound GSOC subscriptions", () => {
    ...
    (mgr as any).inboundGsocSubs.set("sub1", () => cancels.push("sub1"));
    (mgr as any).inboundGsocSubs.set("sub2", () => cancels.push("sub2"));
    mgr.shutdown();
    expect(cancels.sort()).toEqual(["sub1", "sub2"]);
    expect((mgr as any).inboundGsocSubs.size).toBe(0);
  });
});
