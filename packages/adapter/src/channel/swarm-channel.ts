...
export class SwarmChannel implements IChannel {
  ...
  private async pushSyncOperation(syncOp: SyncOperation): Promise<void> {
    ...
    this.logger.verbose(
      `[SwarmChannel] Push ${docId.slice(0, 8)} scope=${scope} idx=${startIndex}-${endIndex} ops=${ops.length}`,
    );
    ...
    if (driveId) {
      const collab = (globalThis as any).__swarmCollabManager__ as
        | { handleLocalPush?: (input: any) => Promise<void> }
        | undefined;
      if (collab?.handleLocalPush) {
        void collab.handleLocalPush({
          ...
          this.logger.warn(
            `[SwarmChannel] Collab mirror hook rejected: ${err instanceof Error ? err.message : err}`,
          );
        });
      }
    }
  ...
  async pullFromSwarm(): Promise<boolean> {
    ...
    if (!this.swarmClient) {
      ...
      if (!this.swarmClient) {
        this.logger.warn(
          `[SwarmChannel] pullFromSwarm: swarmClient unresolved, skipping`,
        );
        return false;
      }
    }
    ...
    for (const docId of orderedDocIds) {
      ...
      try {
        ...
        if (docTypeForId === "powerhouse/document-drive") {
          ...
          if (hasDelete) {
            this.logger.info(
              `[SwarmChannel] Deleted drive ${docId.slice(0, 8)} detected — running one-time cleanup`,
            );
   ...
   * reactor state.
   */
  private findDriveIdForThisRemote(): string | null {
