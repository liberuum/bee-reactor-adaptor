 ...
 */
export async function ensureCollabInUserManifest(
  client: SwarmClient,
  ownerAddress: string,
  ...
  return enqueueUserManifestWrite(ownerAddress, async () => {
    ...
    const lastActivityAt =
      prior && prior.lastActivityAt > entry.lastActivityAt
    ...
    await writeUserManifestAndCache(client, ownerAddress, manifest);
    console.log(
      `[ManifestManager] Collab ${prior ? "updated" : "added"} in user manifest: ${entry.collabId}`,
    );
  });
}
 ...
 * explicitly revoke).
 */
export async function removeCollabFromUserManifest(
  client: SwarmClient,
  ownerAddress: string,
  collabId: string,
): Promise<void> {
  return enqueueUserManifestWrite(ownerAddress, async () => {
    const manifest = await readUserManifestCached(client, ownerAddress);
    if (!manifest?.collabs?.[collabId]) return;
    const collabs = { ...manifest.collabs };
 ...
 */
export async function listCollabsFromUserManifest(
  client: SwarmClient,
  ownerAddress: string,
): Promise<Record<string, UserCollabEntry>> {
  const manifest = await readUserManifestCached(client, ownerAddress);
