...
import type { CollabOpsBatch, CollabOpsFeed } from "../collab-ops-feed.js";
...
export class CollabFeedFlusher {
  ...
  queue(summary: CollabSummary, input: QueueInput): void {
    ...
    const merged: PendingBatch = existing
  ...
  private async flushOne(key: string): Promise<void> {
    ...
    try {
      ...
      if (!granteeHistRef) {
        ...
        this.pending.set(
          key,
          current ? { ...current, ops: [...entry.ops, ...current.ops] } : entry,
        );
        return;
      }
      ...
      void this.gsoc
        .pingOpCommitted(
          this.store.get(entry.summary.collabId) ?? entry.summary,
          entry.driveId,
          entry.docId,
          {
            ...writeResult,
        ...
        .catch(() => { /* already logged inside pingOpCommitted */ });
    } catch (err) {
      console.warn(
        `[CollabManager] mirror flush failed for ${entry.summary.collabId} doc ${entry.docId.slice(0, 8)}:`,
...
function batchKey(collabId: CollabId, driveId: string, docId: string): string {
