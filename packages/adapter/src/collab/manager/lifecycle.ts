 ...
 */
function displayNameFromProfile(profile: SwarmPublicProfile): string | undefined {
  const candidate = (profile as SwarmPublicProfile & { ensName?: unknown }).ensName;
...
export class CollabLifecycle {
  ...
  async accept(invite: CollabInviteAttachment): Promise<CollabSummary> {
    ...
    const { participants, manifestFeedIndex } = await this.resolveAcceptParticipants(
      invite,
      mySelf,
      now,
    );
   ...
   * behavior.
   */
  private async downloadInitialBundle(
    invite: CollabInviteAttachment,
  ): Promise<Uint8Array> {
   ...
   */
  private async resolveAcceptParticipants(
    invite: CollabInviteAttachment,
    mySelf: CollabParticipant,
    now: string,
  ): Promise<{ participants: CollabParticipant[]; manifestFeedIndex?: number }> {
    try {
      const latest = await this.manifestFeed.readLatest(
        invite.collabId,
        invite.invitedBy,
  ...
  async leave(collabId: CollabId): Promise<void> {
    if (!this.store.has(collabId)) return;
  ...
  private buildManifestRevision(
    base: CollabSummary,
    overrides: { participants: CollabParticipant[]; now: string },
  ): CollabManifest {
    return {
      version: 1,
      ...
      createdAt:
        base.participants.find((p) => p.address === base.initiator)?.joinedAt
        ?? overrides.now,
      updatedAt: overrides.now,
    };
  }

  private async resolveParticipantProfiles(
    addresses: string[],
  ): Promise<Array<{ address: string; beeNodePublicKey: string; displayName?: string }>> {
    const out: Array<{ address: string; beeNodePublicKey: string; displayName?: string }> = [];
...
function dedupeParticipants(participants: string[], myAddress: string): string[] {
