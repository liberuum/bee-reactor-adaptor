...
export class SwarmClient {
  ...
  async uploadData(
    ...
    try {
      ...
      if (err?.status === 404 || /batch with id not found/i.test(String(err?.responseBody ?? err?.message ?? ""))) {
        console.warn(
          `[SwarmClient] uploadData(/bytes) 404 with batchId=${JSON.stringify(this.batchId)} (type=${typeof this.batchId}, length=${(this.batchId as string)?.length})`,
        );
      }
      throw err;
    }
  ...
  async createGrantees(publicKeys: string[]): Promise<{ ref: string; historyRef: string }> {
    ...
    const data = (await res.json()) as { ref: string; historyref: string };
   ...
   */
  async readFeedReference(topic: Topic, ownerAddress: string): Promise<string | null> {
    try {
      const reader = this.bee.makeFeedReader(topic, ownerAddress);
      const result = await reader.downloadReference();
