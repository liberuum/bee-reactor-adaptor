...
var ReactorClient = class {
	...
	*/
	async getOperations(documentIdentifier, view, filter, paging, signal) {
		...
		const allOperations = [];
		for (const scopeResults of scopeEntries) allOperations.push(...scopeResults.results);
		...
		return {
			...
			nextCursor: scopeEntries.length === 1 ? scopeEntries[0].nextCursor : void 0
		};
	}
	/**
	...
	async deleteDocument(identifier, propagate, signal) {
		...
		if (propagate === PropagationMode.Cascade) {
			...
			while (queue.length > 0) {
				...
				if (signal?.aborted) throw new Error("Operation aborted");
				const relationships = await this.documentIndexer.getOutgoing(currentId, ["child"], void 0, void 0, signal);
				for (const rel of relationships.results) if (!visited.has(rel.targetId)) {
	...
	async removeFromAllParents(documentId, signal) {
		...
		for (const rel of incoming.results) {
			...
			if (isDrive) {
				...
				const jobInfo = await this.reactor.removeChildren(parentId, [documentId], "main", this.signer, signal);
				jobs.push(jobInfo);
			}
		}
		return jobs;
